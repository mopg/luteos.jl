function hdgSolveElas( master::Master2D, mesh::Mesh2D, material::Material, source::Function, bc::Array{Function})
...
Cstiff = compCstiff( material, dim )
...
for pp in 1:nelem # Loop over all elements
  ...
  for ii in 1:dd, jj in 1:dd
    ij = (ii-1)*dd + jj
    for kk in 1:dd, ll in 1:dd
      kl = (kk-1)*dd + ll
      M[1+(ij-1)*nnodes:ij*nnodes,1+(kl-1)*nnodes:kl*nnodes,pp] -= Cstiff[ii,jj,kk,ll] * master.phi * jcwd * phi'
    end
  end
  ...
  for qq in 1:(dim+1) # loop over number of faces
    ...
    if mesh.t2f[pp,qq] < 0
      ...
      nod    = 1 + (qq-1)*szF:qq*szF
      rotdir = true
    else
      # face follows counter-clockwise rotation
      nod    = qq*szF:-1:1 + (qq-1)*szF
      rotdir = false
    end

    p1d  = master.phi1d'  * mesh.dgnodes[nod,:,pp]
    dp1d = master.dphi1d' * mesh.dgnodes[nod,:,pp]

    jac1d  = sqrt( dp1d[:,1].^2 + dp1d[:,2].^2 )
    jcw1d  = master.gw1d .* jac1d
    jcw1dd = diagm( jcw1d )

    if rotdir
      nL =  [ p1d_xi[:,2] -p1d_xi[:,1] ] ./ [jac1d jac1d]
    else
      nL = -[ p1d_xi[:,2] -p1d_xi[:,1] ] ./ [jac1d jac1d]
    end
    ...
    for ii in 1:dd, jj in 1:dd
      ij = (ii-1)*dd + jj
      A[(ii-1)*nnodes + nod,(ij-1)*nnodes + nod,pp] -= master.phi1d * jcw1dd * ( diagm(nL[:,jj]) *master.phi1d )'
    end

    ## B
    # -<v_i, tau_{ijkl} u^h_k * n_l * n_j >_{dT^h}
    for ii in 1:dd, jj in 1:dd, kk in 1:dd, ll in 1:dd #l
      B[(ii-1)*nnodes + nod,(kk-1)*nnodes + nod,pp] +=
        tau[ii,jj,kk,ll] * master.phi1d * jcw1dd * (diagm(nL[:,ll].*nL[:,jj]) * master.phi1d)'
    end

    ## C
    # <v_i, tau_{ijkl} \hat{u}^h_k * n_l * n_j >_{dT^h}
    for ii in 1:dd, jj in 1:dd, kk in 1:dd, ll in 1:dd
      C[(ii-1)*nnodes + nod,(qq-1)*szF + (kk-1)*szF + faceInd,pp] -=
          tau[ii,jj,kk,ll] * shap1d * jcw1dd * (diagm(nL[:,ll] .* nL[:,jj]) * master.phi1d)'
    end
    ...
    for ii in 1:dd, jj in 1:dd
      ij = (ii-1)*dd + jj
      J[(ij-1)*nnodes + nod,(qq-1)*szF + (ii-1)*szF + faceInd,pp] -= 0.5*shap1d * jcw1dd * (diagm(nL[:,jj]) * master.phi1d)'
      J[(ij-1)*nnodes + nod,(qq-1)*szF + (jj-1)*szF + faceInd,pp] -= 0.5*shap1d * jcw1dd * (diagm(nL[:,ii]) * master.phi1d)'
    end
    ...
    if ( mesh.f[ indF, 4 ] < 0 )
      ...
      if problem.bctype[bNo] == 1
        ...
        for ii in 1:dd     %i
          G((ii-1)*szF + (qq-1)*szF + faceInd, 1,pp) = shap1d * jcw1dd * problem.bcfunc[bNo,ii](p1d)
        end
      ...
      elseif problem.bctype[bNo] == 2
        ...
        if problem.bcnorm
          ...
          for ii in 1:dd     #i
              temp = ( nL[:,ii] .* problem.bcfunc[bNo,1](p1d) + tL[:,ii] .* problem.bcfunc[bNo,2](p1d) )
      ...
      for ii in 1:dd, jj in 1:dd
        ij = (ii-1)*dd + jj
        N[(ii-1)*szF + (qq-1)*szF + faceInd, (ij-1)*nnodes + nod,pp] += master.phi * jcw1dd * (diagm(nL[:,jj]) * master.phi1d)'
      end
