...
function ExactSol( Cstiff )
  ...
  u1 = (x2 - x2.^2) .* cos.(pi*x1) .* (1/2 - x1) .* (1 - x2/2)   #10 * (x2 - x2.^2) .* sin.(pi*x1) .* (1 - x1) .* (1 - x2/2)
  u2 = 2 * (x1 - x1.^2) .* sin.(pi*x2) .* (1 - x2) .* (1 - x1/2) # 2 * (x1 - x1.^2) .* sin.(pi*x2) .* (1 - x2) .* (1 - x1/2)
...
funcNE = (p) -> [ σ1func(p)  σ2func(p)]
funcNW = (p) -> [-σ1func(p) -σ2func(p)]
bctype = [1,2,1,2] # All Dirichlet
