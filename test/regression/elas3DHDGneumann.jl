...
function ExactSol3D( Cstiff )
  ...
  u3 =  5 * (x1 - x1.^2) .* (x2 - x2.^2) .* cos.(pi*x3) .* (1 - x3/2) .* (1 - x1/2) .* (1 - x2/2)
...
bctype = [2,1,2,1,1,1]
funcTop = (p) -> [ σ13func(p)  σ23func(p)  σ33func(p)]
funcBot = (p) -> [-σ13func(p) -σ23func(p) -σ33func(p)]
...
for ii in 1:length(Ps), jj in 1:length(Ns)
  ...
  prob = Elas( @sprintf("Reg %i %i", P.p, N), mat, source, bctype, false, [funcBot, funcB, funcTop, funcB, funcB, funcB] )
...
# Output to terminal
@printf("\n")
@printf("   Convergence rates for 3D Dirichlet + Neumann problem\n\n")
@printf("   ------------------------------------------\n\n")
@printf( "P    ")
...
open("errors_Elas_Neumann3D.dat", "w") do f
